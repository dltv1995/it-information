(() => {
  ...
  const customPickerState={start:{hour:null,minute:null},end:{hour:null,minute:null}}; let customDateView=new Date(2026,8,1);
  function setupEventPickers(){setupDatePicker();setupTimePicker("eventStartTime","start");setupTimePicker("eventEndTime","end");}
  function setupDatePicker(){const input=document.getElementById("eventDate");if(!input)return;input.type="hidden";const root=document.createElement("div");root.className="custom-date-picker";root.innerHTML='<button type="button" class="custom-picker-trigger"><strong>เลือกวันที่</strong><i class="fa-regular fa-calendar"></i></button><div class="custom-date-pop hidden"></div>';input.before(root);root.appendChild(input);root.querySelector('button').onclick=e=>{e.stopPropagation();closeCustomPickers();const d=input.value?parseDateKey(input.value):new Date();customDateView=new Date(d.getFullYear(),d.getMonth(),1);drawDatePicker(root);root.querySelector('.custom-date-pop').classList.remove('hidden')}}
  function drawDatePicker(root){const input=root.querySelector('#eventDate'),pop=root.querySelector('.custom-date-pop'),y=customDateView.getFullYear(),m=customDateView.getMonth(),first=new Date(y,m,1),start=new Date(y,m,1-first.getDay());let days='';for(let i=0;i<42;i++){const d=new Date(start);d.setDate(start.getDate()+i);const key=toDateKey(d);days+=`<button type="button" data-date="${key}" class="${d.getMonth()!==m?'outside ':''}${key===input.value?'selected':''}">${d.getDate()}</button>`}pop.innerHTML=`<header><strong>${MONTHS_TH[m]} ${y+543}</strong><span><button type="button" data-prev>‹</button><button type="button" data-next>›</button><button type="button" data-close>×</button></span></header><div class="custom-week">${['อา','จ','อ','พ','พฤ','ศ','ส'].map(x=>`<b>${x}</b>`).join('')}</div><div class="custom-days">${days}</div><footer><button type="button" data-clear>ล้าง</button><button type="button" data-today>วันนี้</button></footer>`;pop.onclick=e=>e.stopPropagation();pop.querySelector('[data-close]').onclick=()=>pop.classList.add('hidden');pop.querySelector('[data-prev]').onclick=()=>{customDateView.setMonth(customDateView.getMonth()-1);drawDatePicker(root)};pop.querySelector('[data-next]').onclick=()=>{customDateView.setMonth(customDateView.getMonth()+1);drawDatePicker(root)};pop.querySelector('[data-clear]').onclick=()=>{input.value='';syncPickerDisplays();pop.classList.add('hidden')};pop.querySelector('[data-today]').onclick=()=>{input.value=toDateKey(new Date());syncPickerDisplays();pop.classList.add('hidden')};pop.querySelectorAll('[data-date]').forEach(b=>b.onclick=()=>{input.value=b.dataset.date;syncPickerDisplays();pop.classList.add('hidden')})}
  function setupTimePicker(id,kind){const input=document.getElementById(id);if(!input)return;input.type="hidden";const root=document.createElement('div');root.className=`custom-time-picker ${kind==='end'?'end':''}`;root.innerHTML='<button type="button" class="custom-picker-trigger"><strong>--:--</strong><i class="fa-regular fa-clock"></i></button><div class="custom-time-pop hidden"></div>';input.before(root);root.appendChild(input);root.querySelector('button').onclick=e=>{e.stopPropagation();closeCustomPickers();const [h,n]=(input.value||'').split(':');customPickerState[kind]={hour:h||null,minute:n||null};drawTimePicker(root,input,kind);root.querySelector('.custom-time-pop').classList.remove('hidden')}}
  function drawTimePicker(root,input,kind){const pop=root.querySelector('.custom-time-pop'),st=customPickerState[kind],hours=Array.from({length:24},(_,i)=>String(i).padStart(2,'0')),mins=Array.from({length:12},(_,i)=>String(i*5).padStart(2,'0'));pop.innerHTML=`<header><strong>${kind==='start'?'เวลาเริ่ม แบบ 24 ชั่วโมง':'เวลาสิ้นสุด แบบ 24 ชั่วโมง'}</strong><button type="button" data-close>×</button></header><div class="custom-time-preview">${st.hour||'--'} : ${st.minute||'--'}</div><small>เลือกชั่วโมง</small><div class="custom-hour-grid">${hours.map(v=>`<button type="button" data-hour="${v}" class="${st.hour===v?'active':''}">${v}</button>`).join('')}</div><small>เลือกนาที</small><div class="custom-minute-grid">${mins.map(v=>`<button type="button" data-minute="${v}" class="${st.minute===v?'active':''}">${v}</button>`).join('')}</div><button type="button" class="custom-apply" ${!st.hour||!st.minute?'disabled':''}>ใช้เวลานี้</button>`;pop.onclick=e=>e.stopPropagation();pop.querySelector('[data-close]').onclick=()=>pop.classList.add('hidden');pop.querySelectorAll('[data-hour]').forEach(b=>b.onclick=()=>{st.hour=b.dataset.hour;drawTimePicker(root,input,kind)});pop.querySelectorAll('[data-minute]').forEach(b=>b.onclick=()=>{st.minute=b.dataset.minute;drawTimePicker(root,input,kind)});pop.querySelector('.custom-apply').onclick=()=>{if(!st.hour||!st.minute)return;input.value=`${st.hour}:${st.minute}`;syncPickerDisplays();pop.classList.add('hidden')}}
  function syncPickerDisplays(){const d=document.getElementById('eventDate'),dt=document.querySelector('.custom-date-picker .custom-picker-trigger strong');if(dt)dt.textContent=d?.value?formatThaiDate(d.value):'เลือกวันที่';['eventStartTime','eventEndTime'].forEach(id=>{const input=document.getElementById(id),t=input?.closest('.custom-time-picker')?.querySelector('.custom-picker-trigger strong');if(t)t.textContent=input.value||'--:--'})}
  function closeCustomPickers(){document.querySelectorAll('.custom-date-pop,.custom-time-pop').forEach(x=>x.classList.add('hidden'))}
  function bindEvents(){
    ...
    document.addEventListener("click",e=>{
      ...
      if(e.target.classList.contains("modal"))closeModal(e.target.id); if(!e.target.closest(".custom-date-picker,.custom-time-picker"))closeCustomPickers();
    });
