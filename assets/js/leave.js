...
console.log('leave.js loaded: simple-header-v3');
...
async function checkPermission(user, uid, action) {
  ...
  try {
    ...
    console.warn('override check failed:', error);
  }
  const roleDefaults = {
...
async function initLeaveSystem(user, uid) {
  ...
  if (!ready) {
    showAlert('โหลดข้อมูลไม่สำเร็จ: ไม่พบโครงสร้างหน้า leaveTemplate หลังโหลด header', 'error');
    return;
  }
...
function setupTabs() {
  const tabMy = byId('tabMyLeaves');
  const tabAp = byId('tabApprovals');
  const viewMy = byId('viewMyLeaves');
  const viewAp = byId('viewApprovals');
  ...
  tabMy.addEventListener('click', () => {
    viewMy.classList.remove('hidden'); viewAp.classList.add('hidden');
    tabMy.classList.add(...active); tabMy.classList.remove(...inactive);
    tabAp.classList.add(...inactive); tabAp.classList.remove(...active);
  });
  tabAp.addEventListener('click', () => {
    viewMy.classList.add('hidden'); viewAp.classList.remove('hidden');
    tabAp.classList.add(...active); tabAp.classList.remove(...inactive);
    tabMy.classList.add(...inactive); tabMy.classList.remove(...active);
  });
}

function setupLeaveModal() {
  const modal = byId('leaveModal');
  const content = byId('leaveModalContent');
  const form = byId('leaveForm');
  if (!modal || !content || !form || form.dataset.bound) return;
  form.dataset.bound = '1';
  const close = () => {
    modal.classList.add('opacity-0'); content.classList.add('scale-95'); document.body.style.overflow = '';
    setTimeout(() => { modal.classList.add('hidden'); modal.classList.remove('flex'); }, 180);
    form.reset(); byId('leaveModalError')?.classList.add('hidden');
  };
  const open = () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const err = byId('leaveModalError');
    const btn = byId('saveLeaveBtn');
    ...
    try {
      ...
      await addDoc(collection(db, 'leaves'), {
        ...
        type, startDate: Timestamp.fromDate(startDate), endDate: Timestamp.fromDate(endDate), totalDays, reason,
      ...
      if (err) { err.textContent = error.message || 'เกิดข้อผิดพลาดในการบันทึกข้อมูล'; err.classList.remove('hidden'); }
    } finally {
...
function dateRange(data) {
  const s = data.startDate?.toDate ? data.startDate.toDate().toLocaleDateString('th-TH') : '-';
  const e = data.endDate?.toDate ? data.endDate.toDate().toLocaleDateString('th-TH') : '-';
  return s === e ? s : `${s} - ${e}`;
}
...
async function loadPendingApprovals() {
  ...
  const tbody = byId('approvalsTableBody'); const badge = byId('pendingBadge'); if (!tbody) return;
  try {
...
async function deductQuota(id) {
  ...
  if (leave.type === 'annual') {
    const next = Math.max(Number(quota(user, 'annual')) - Number(leave.totalDays || 0), 0);
    await updateDoc(userRef, { 'leaveQuota.annual': next, 'leave_quota.annual': next });
  }
  if (leave.type === 'sick') {
    const next = Math.max(Number(quota(user, 'sick')) - Number(leave.totalDays || 0), 0);
    await updateDoc(userRef, { 'leaveQuota.sick': next, 'leave_quota.sick': next });
  }
}
