...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function checkPermission(user, uid, action) {
    ...
    try {
      ...
      if (
        overrideDoc.exists() &&
        overrideDoc.data().overrides &&
        overrideDoc.data().overrides[action]
      ) {
  ...
  async function initLeaveSystem(user, uid) {
    ...
    // Setup Quota Display
    document.getElementById("quotaAnnual").textContent =
      user.leave_quota?.annual ?? 10;
    document.getElementById("quotaSick").textContent =
      user.leave_quota?.sick ?? 30;
  ...
  function setupLeaveModal() {
    ...
    document
      .getElementById("requestLeaveBtn")
      .addEventListener("click", () => toggleModal(true));
    document
      .getElementById("closeLeaveModalBtn")
      .addEventListener("click", () => toggleModal(false));
    document
      .getElementById("cancelLeaveModalBtn")
      .addEventListener("click", () => toggleModal(false));
  ...
  async function loadMyLeaves(uid) {
    ...
    try {
      ...
      leaves.forEach((data) => {
        ...
        const statuses = {
          pending:
            '<span class="inline-block px-3 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-xs font-medium border border-amber-200 dark:border-amber-800/50">รออนุมัติ</span>',
          approved:
            '<span class="inline-block px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-medium border border-emerald-200 dark:border-emerald-800/50">อนุมัติแล้ว</span>',
          rejected:
            '<span class="inline-block px-3 py-1 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs font-medium border border-red-200 dark:border-red-800/50">ไม่อนุมัติ</span>',
        };
    ...
    try {
      ...
      document.querySelectorAll(".approve-btn").forEach((btn) => {
        btn.addEventListener("click", (e) =>
          handleApproval(e.target.getAttribute("data-id"), "approved"),
        );
      });
      document.querySelectorAll(".reject-btn").forEach((btn) => {
        btn.addEventListener("click", (e) =>
          handleApproval(e.target.getAttribute("data-id"), "rejected"),
        );
      });
    } catch (error) {
  ...
  async function handleApproval(leaveId, newStatus) {
    if (
      confirm(
        `ยืนยันการ${newStatus === "approved" ? "อนุมัติ" : "ปฏิเสธ"}ใบลา?`,
      )
    ) {
      try {
            ...
            if (uDoc.exists()) {
              ...
              if (lData.type === "annual" && uData.leave_quota?.annual) {
                await updateDoc(uDocRef, {
                  "leave_quota.annual":
                    uData.leave_quota.annual - lData.totalDays,
                });
              } else if (lData.type === "sick" && uData.leave_quota?.sick) {
                await updateDoc(uDocRef, {
                  "leave_quota.sick": uData.leave_quota.sick - lData.totalDays,
                });
              }
            }
          }
