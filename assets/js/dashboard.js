...
// Version: dashboard-project-list-match-chart-v29-fix
...
console.log('dashboard.js loaded: dashboard-project-list-match-chart-v29-fix');
...
let mounted = false;
let dashboardProjectResizeSyncBound = false;

document.addEventListener('DOMContentLoaded', initDashboardPage);

function initDashboardPage() {
    if (!dashboardProjectResizeSyncBound) {
        dashboardProjectResizeSyncBound = true;
        window.addEventListener('resize', () => scheduleProjectSectionHeightSync());
    }
    showBody();
    waitForPageContent(() => {
...
function getSectionChartTone(section, kind) {
    const toneMap = {
        technical: {
            used: '#2563eb',
            remaining: '#93c5fd'
    ...
    const tone = toneMap[section] || toneMap.unknown;
    return tone[kind] || tone.used;
}

function getSectionChartLabel(row, kind) {
    const prefix = kind === 'used' ? 'ใช้แล้ว' : 'คงเหลือ';
...
function updateDashboardSectionTitles() {
    ...
    if (budgetTitle) budgetTitle.textContent = 'สัดส่วนงบประมาณทั้งหมด ใช้แล้ว/คงเหลือตามส่วนงาน';
...
function injectDashboardPolishStyle() {
    ...
    style.textContent = `
        ...
        .budget-section-summary .text-amber-600,
        .budget-section-summary .text-emerald-600 {
            letter-spacing: .01em;
        }
...
function syncProjectSectionHeightToChart() {
    const projectList = document.getElementById('projectList');
    const projectSection = projectList?.closest('section');
    const chartCanvas = document.getElementById('budgetChart');
    const chartSection = chartCanvas?.closest('section');
    if (!projectSection || !chartSection || projectSection === chartSection) return;

    if (window.innerWidth < 1280) {
        projectSection.style.height = '';
        projectSection.style.maxHeight = '';
        projectSection.style.display = '';
        projectSection.style.flexDirection = '';
        projectSection.style.overflow = '';
        projectList.style.maxHeight = '';
        projectList.style.overflowY = '';
        projectList.style.flex = '';
        return;
    }

    const chartHeight = Math.max(520, Math.round(chartSection.getBoundingClientRect().height));
    projectSection.style.height = `${chartHeight}px`;
    projectSection.style.maxHeight = `${chartHeight}px`;
    projectSection.style.display = 'flex';
    projectSection.style.flexDirection = 'column';
    projectSection.style.overflow = 'hidden';

    const header = projectSection.querySelector('.px-6.py-5') || projectSection.firstElementChild;
    const headerHeight = header ? Math.round(header.getBoundingClientRect().height) : 0;
    const listHeight = Math.max(260, chartHeight - headerHeight);
    projectList.style.flex = '1 1 auto';
    projectList.style.maxHeight = `${listHeight}px`;
    projectList.style.overflowY = 'auto';
}

function scheduleProjectSectionHeightSync() {
    requestAnimationFrame(() => {
        syncProjectSectionHeightToChart();
        setTimeout(syncProjectSectionHeightToChart, 120);
        setTimeout(syncProjectSectionHeightToChart, 320);
    });
}
...
function renderDashboard(data) {
    ...
    scheduleProjectSectionHeightSync();
        ...
        return `
            <article class="budget-section-card" style="border-color:${escapeAttr(colorWithAlpha(row.color, '88'))}; box-shadow: inset 4px 0 0 ${escapeAttr(row.color)}">
                ...
                <div class="space-y-2 text-xs">
                    ...
                    <div class="flex justify-between gap-3"><span class="text-slate-500 dark:text-slate-400">ใช้แล้ว/อนุมัติแล้ว</span><b style="color:${escapeAttr(row.color)}">${baht(row.approved)}</b></div>
    ...
    panel.innerHTML = `
        <div class="mb-3 flex flex-col md:flex-row md:items-end md:justify-between gap-2">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">งบที่กระจาย / ใช้แล้วหรืออนุมัติแล้ว / งบคงเหลือของแต่ละส่วนงาน</p>
            </div>
        ...
        <div class="budget-section-total-row mt-4 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                ...
                <div class="text-sm font-extrabold text-slate-900 dark:text-white mt-1">งบรวมทั้งหมด ${baht(stats.totalBudget)}</div>
            </div>
...
function renderBudgetChart(summary, projects = []) {
    ...
    stats.rows.forEach(row => {
        if (row.approved > 0) {
            chartLabels.push(getSectionChartLabel(row, 'used'));
            chartValues.push(row.approved);
            chartColors.push(getSectionChartTone(row.section, 'used'));
        }
        if (row.remaining > 0) {
            chartLabels.push(getSectionChartLabel(row, 'remaining'));
            chartValues.push(row.remaining);
            chartColors.push(getSectionChartTone(row.section, 'remaining'));
        }
    });
