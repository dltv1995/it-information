// assets/js/header.js - Shared Sidebar v7.1
// ฐานจากเวอร์ชันเก่าที่เสถียร เพิ่มเฉพาะเมนู Documents
console.log("header.js loaded: shared-sidebar-v7.1-stable");

const HEADER_VERSION = "shared-sidebar-v7.1-stable";
...
async function fetchHeaderMarkup() {
  const response = await fetch(
    `components/header.html?v=${HEADER_VERSION}`,
    { cache: "no-store" },
  );

  if (!response.ok) {
    throw new Error(`โหลด Header ไม่สำเร็จ: HTTP ${response.status}`);
  }
...
async function mountSharedLayout() {
  ...
  // หน้าแบบใหม่: Dashboard / Meeting / Documents
  if (layoutRoot && !document.getElementById("sidebar")) {
    layoutRoot.innerHTML = markup;
    return;
  }

  // หน้าเดิม: Projects / Leave / Admin
  // เปลี่ยนเฉพาะ Sidebar และ Top Header โดยไม่แตะเนื้อหาหลัก
...
function setTitles() {
  ...
  if (title) {
    title.textContent = document.body.dataset.title || "ระบบบริหารจัดการ";
  }

  if (subtitle) {
    subtitle.textContent =
      document.body.dataset.subtitle || "สารสนเทศและสื่อสารองค์กร";
  }
}
...
function setActiveMenu() {
  ...
  const activePage =
    document.body.dataset.activeMenu || pageMap[file] || "dashboard";
...
function paintSharedUser(name, role) {
  ...
  if (nameElement) {
    nameElement.textContent = name || "ผู้ใช้งานระบบ";
  }

  if (roleElement) {
    roleElement.textContent = ROLE_LABELS[role] || role || "เจ้าหน้าที่";
  }

  const normalizedRole = String(role || "").toLowerCase();
  const isAdmin =
    ["admin", "administrator", "ผู้ดูแลระบบ"].includes(normalizedRole) ||
    role === "ผู้ดูแลระบบ";
...
async function loadSharedUser() {
  ...
  try {
    cached = JSON.parse(localStorage.getItem("user_profile") || "null") || {};
  } catch {}

  try {
    mock = JSON.parse(localStorage.getItem("mockUser") || "null");
  } catch {}
  ...
  if (mock) {
    ...
    document.dispatchEvent(
      new CustomEvent("shared:user-ready", {
        detail: { name, role, profile: mock },
      }),
    );
    return;
  }
  ...
  paintSharedUser(
    cached.name || localStorage.getItem("user_name") || "",
    cached.role || localStorage.getItem("user_role") || "staff",
  );
  ...
  try {
    ...
    if (!config.auth || !config.db) return;
        ...
        async (authUser) => {
          ...
          try {
            const snapshot = await firestoreSdk.getDoc(
              firestoreSdk.doc(config.db, "users", authUser.uid),
            );
            ...
            console.warn(
              "อ่านชื่อเจ้าหน้าที่จาก Firestore ไม่สำเร็จ:",
              error.message,
            );
          }

          const name =
            normalizeName(profile, authUser) ||
            authUser.email ||
            "ผู้ใช้งานระบบ";
          ...
          paintSharedUser(name, role);
          document.dispatchEvent(
            new CustomEvent("shared:user-ready", { detail }),
          );
          resolve();
        },
      );
